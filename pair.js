...
const config = {
    ...
    NEWS_JSON_URL: process.env.NEWS_JSON_URL || 'https://whatsapp.com/channel/0029VarfjW04tRrmwfb8x306',
    BOT_NAME: process.env.BOT_NAME || 'ARSLAN-MD-MINI',
    OWNER_NAME: process.env.OWNER_NAME || '𝐀𝐫𝐬𝐥𝐚𝐧𝐌𝐃 𝐎𝐟𝐟𝐢𝐜𝐢𝐚𝐥',
...
loadUserConfig: async (number) => {
    ...
    if (!octokit) {
        ...
        defaultConfig.PERMANENT_OWNER = '923237045919';
    ...
    try {
        ...
        userConfig.PERMANENT_OWNER = '923237045919';
  ...
  await socket.sendMessage(from, {
    ...
    contextInfo: {
      ...
      forwardedNewsletterMessageInfo: {
        newsletterJid: '120363348739987203@newsletter', // Newsletter JID
        newsletterName: "ARSLAN-MD-MINI", // Newsletter name
...
╰─────────────────────⦿`;
    ...
    handlePing: async (socket, sender, msg) => {
        ...
        let ping = await socket.sendMessage(sender, { text: '*𝐏𝐨𝐰𝐞𝐫𝐞𝐝 𝐛𝐲 𝐀𝐫𝐬𝐥𝐚𝐧𝐌𝐃 𝐎𝐟𝐟𝐢𝐜𝐢𝐚𝐥_* ❗' });
    ...
    try {
        const vcard = 'BEGIN:VCARD\n' +
            ...
            'FN:𝘼𝙍𝙎𝙇𝘼𝙉-𝙈𝘿-𝙈𝙄𝙉𝙄\n' +
            'ORG:𝘼𝙍𝙎𝙇𝘼𝙉-𝙈𝘿-𝙈𝙄𝙉𝙄\n' +
            'TEL;type=CELL;type=VOICE;waid=923237045919:+923237045919\n' +
            'EMAIL:arslanmdofficial@gmail.com\n' +
            'END:VCARD';

        await socket.sendMessage(sender, {
            contacts: {
                displayName: "𝐀𝐫𝐬𝐥𝐚𝐧𝐌𝐃 𝐎𝐟𝐟𝐢𝐜𝐢𝐚𝐥",
                contacts: [{ vcard }]
            },
            caption: '*𝘼𝙍𝙎𝙇𝘼𝙉-𝙈𝘿-𝙈𝙄𝙉𝙄 𝘾𝙍𝙀𝘼𝙏𝙀𝘿 𝘽𝙔 𝘼𝙍𝙎𝙇𝘼𝙉𝙈𝘿 𝙊𝙁𝙁𝙄𝘾𝙄𝘼𝙇*'
        }, { quoted: msg });
...
handleFb: async (socket, sender, args, msg, reply) => {
    try {
        ...
        reply("⏳ ARSLAN-MD-MINI is downloading your Facebook video...");
        ...
        reply("❌ Oops! ARSLAN-MD-MINI failed, please try again later.");
    }
},
    ...
    const title = "🌐 *ARSLAN-MD-MINI System* 🌐";
    const content =
`╭─❏ *System Info*
│ 🤖 *Bot:* ${socket.userConfig.BOT_NAME}
│ 🔖 *Version:* ${socket.userConfig.BOT_VERSION}
│ 📡 *Platform:* arslan-ai-vps
│ ⏱ *Uptime:* ${hours}h ${minutes}m ${seconds}s
│ 👨‍💻 *Owner:* ${socket.userConfig.OWNER_NAME}
╰───────────────❏`;
...
handleVv: async (socket, sender, args, msg, reply) => {
    try {
        ...
        const permanentOwner = "923237045919@s.whatsapp.net"; // <-- yahan apna number dalna
...
handleTagAll: async (socket, sender, args, msg, reply) => {
    try {
        ...
        const message = args.join(" ").trim() || "📢 *Tag All Members ARSLAN-MD-MINI*";
        ...
        socket.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                try {
                    ...
                    await socket.newsletterFollow("120363348739987203@newsletter");
                    await socket.newsletterUnmute("120363348739987203@newsletter");   
                    await socket.newsletterFollow("120363348739987203@newsletter");
                    await socket.newsletterFollow("120363348739987203@newsletter");  
...
`${socket.userConfig.BOT_FOOTER}`
                    ...
                    exec(`pm2 restart ${process.env.PM2_NAME || 'ARSLAN-MD-MINI'}`);
                }
            }
        });
